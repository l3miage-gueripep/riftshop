...
app.post('/create_order', (req, res) => {
  const order_data = JSON.parse(req.body.orderContent);
  //get the total price of the order and convert it into a string while keeping only 2 decimals
  let totalPrice = order_data.reduce((acc: number, cartItem: CartItem) => acc + (cartItem.product.price * cartItem.quantity), 0).toFixed(2);
  get_access_token()
    .then(access_token => {
      let order_data_json = {
